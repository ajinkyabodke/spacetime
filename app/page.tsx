...
function SpaceTimeGrid({ objects, onPlaneReady }: SpaceTimeGridProps) {
  ...
  useFrame(() => {
    ...
    const positions = (gridRef.current.geometry.attributes.position as THREE.BufferAttribute).array
    ...
    for (let i = 0; i < positions.length; i += 3) {
      ...
      objects.forEach(obj => {
        ...
        // Exponential falloff
        const falloff = Math.exp(-horizontalDistance * horizontalDistance * 0.3)
...
function CelestialObject({
  ...
  const getPlaneY = (x: number, z: number): number => {
    ...
    const posAttr = planeMesh.geometry.attributes.position as THREE.BufferAttribute
  ...
  return (
    <group>
      ...
      {selected && (
        <Html position={[position[0], position[1] + props.radius + 1, position[2]]}>
          <button
            onClick={(e) => {
              e.stopPropagation()
              onDelete()
...
export default function Home() {
  ...
  const updateObjectPosition = (id: number, newPosition: [number, number, number]) => {
        ...
        <Canvas camera={{ position: [0, 15, 15], fov: 50 }}>
          ...
          {objects.map(obj => (
            <CelestialObject
              ...
              onPositionChange={(newPosition) => updateObjectPosition(obj.id, newPosition)}
              objects={objects}
